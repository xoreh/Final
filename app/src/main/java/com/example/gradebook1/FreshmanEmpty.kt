...
class FreshmanEmpty : AppCompatActivity() {
    ...
    private lateinit var  sqLiteHelper: SQLiteHelper1E
    private lateinit var recyclerView: RecyclerView

    //THESE VARIABLES ARE PRIVATE AND MAKE SURE THEY ARE NULL
    private var adapter: StudentAdapter1E? = null
    private var std:StudentModel? = null

    //THIS FUNCTION CALLS FOR THE CREATION OF THE LAYOUT
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_freshman_empty)

        //CALLS THE FOLLOWING FUNCTIONS
        initView()
        initRecyclerView()

        sqLiteHelper = SQLiteHelper1E(this)
    ...
    private fun getStudent(){
        val stdList5 = sqLiteHelper.getAllStudent()
        Log.e("pppp","${stdList5.size}")

        //SEND THE "ITEM/STUDENT" TO RecyclerView
        adapter?.addItems(stdList5)
    }
