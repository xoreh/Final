...
class StudentAdapter2: RecyclerView.Adapter<StudentAdapter2.StudentViewHolder>() {
    private var stdList2: ArrayList<StudentModel> = ArrayList()
    ...
    class StudentViewHolder(var view:View): RecyclerView.ViewHolder(view){
        ...
        fun bindView(std2:StudentModel){
            id.text = std2.id.toString()
            name.text = std2.name
            assignment.text = std2.assignment
            quiz.text = std2.quiz
            test.text = std2.test
            exam.text = std2.exam
            grade.text = std2.grade
        }
    }
}