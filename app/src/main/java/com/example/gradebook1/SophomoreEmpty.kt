...
class SophomoreEmpty : AppCompatActivity() {
    ...
    private lateinit var  sqLiteHelper: SQLiteHelper2E
    private lateinit var recyclerView: RecyclerView

    //THESE VARIABLES ARE PRIVATE AND MAKE SURE THEY ARE NULL
    private var adapter: StudentAdapter2E? = null
    private var std:StudentModel? = null

    //THIS FUNCTION CALLS FOR THE CREATION OF THE LAYOUT
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_sophomore_empty)

        //CALLS THE FOLLOWING FUNCTIONS
        initView()
        initRecyclerView()

        sqLiteHelper = SQLiteHelper2E(this)
    ...
    private fun getStudent(){
        val stdList6 = sqLiteHelper.getAllStudent()
        Log.e("pppp","${stdList6.size}")

        //SEND THE "ITEM/STUDENT" TO RecyclerView
        adapter?.addItems(stdList6)
    }
